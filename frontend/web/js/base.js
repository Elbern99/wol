...
// 1. HEADER ---------
            ...
            btn.click(function(e) {
                ...
                if(cur.parent().hasClass(menuMobile.classes)){
                    ...
                    cur.parent().siblings().removeClass(menuMobile.classes)
                        .find(content).slideUp(menuMobile.delay)
    ...
    /* menuSlider */
    var menuMobileSlider = {
        arrowOption: function() {

            if(!$('.owl-item').last(7).hasClass('active')){
                $('.owl-next').fadeIn(0);
                console.log('last has');
            } else {
                $('.owl-next').fadeOut(0);
                console.log('last not has');
            }
        ...
        init: function(slider) {
            ...
            if(_window_width < _mobile) {
                $(slider).jScrollPane(scrollPaneOption);
            }
            ...
            $(window).resize(function() {
                if(_window_width < _mobile) {
                    ...
                    var element = $(slider).jScrollPane({});
            ...
            /* var sliderOption = {
                ...
                //onDragged: menuMobileSlider.arrowOption ,
                onTranslated: menuMobileSlider.arrowOption,
                responsive:{
                    0:{
                        items:4
                    },
        ...
        openReference: function(btn,parent) {
            $(btn).click(function(e) {
                ...
                setTimeout(function(){
                    ...
                    article.arrowsDisabled();
                    article.arrowsSwitchNext('.reference-popup .right');
                    article.arrowsSwitchPrev('.reference-popup .left');
                }, article.delay+100);
                e.preventDefault();
            });
        },
        arrowsDisabled:function() {

        },
        detectCoordinate: function(cur,parent) {
            var coordinate = parseInt( cur.offset().top, 10);
            $(parent).css('top', coordinate );
        },
        arrowsSwitchNext: function(btnNext) {
            $(btnNext).click(function(e) {
                console.log('next');
            });
        },
        arrowsSwitchPrev: function(btnPrev) {
...
// 3. SIDEBAR WIDGETS ---------
    ...
    $(document).ready(function() {
        //HEADER
        //mobile
        menuMobile.toggleItem($('.mobile-menu .has-drop >a'), '.submenu',$('.mobile-menu .header-menu-bottom-list'));
        //desktop
        menuDesktop.toggleMenu($('.header-desktop .has-drop >a'),$('.header-desktop .submenu'));
        dropDown($('.header-desktop .dropdown-link'), $('.drop-content'));

        //article
        article.closeReference('.icon-close-popup ','.reference-popup');
        article.openReference('.link-ref','.reference-popup');
